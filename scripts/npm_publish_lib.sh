...
python3 -c "import os; import json; p = json.load(open('package.json')); p['name'] = '@kunal-mohta/duckdb-wasm'; p['repository']['url'] = 'https://github.com/kunal-mohta/duckdb-wasm.git'; p['version'] = '1.0.0'; p['publishConfig'] = {}; p['publishConfig']['@kunal-mohta:registry'] = 'https://npm.pkg.github.com'; json.dump(p, open('package.json', 'w'), indent=2, ensure_ascii=False);"
...
npm publish --ignore-scripts ${TAG}
